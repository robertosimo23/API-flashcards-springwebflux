...
public class DeckServiceTest {
    ...
    void updateTest() {
        ...
        var document = DeckDocumentFactoryBot.builder()
                .preUpdate(storedDeck.id(), storedDeck.createdAt(), storedDeck
                        .updatedAt())
                .build();
        when(deckRepository.save(any())).thenAnswer(invocation -> {
            var deck = invocation.getArgument(0, DeckDocument.class);
        ...
        StepVerifier.create(deckService.update(document))
                .assertNext(actual -> {
                    ...
                    assertThat(actual.updatedAt().toEpochSecond()).isEqualTo(storedDeck.updatedAt().toEpochSecond());
                })
                .verifyComplete();
        verify(deckRepository).save(any());
    ...
    @Test
    void whenTryToUpdateStoreDeckThenThrowError() {
        ...
        StepVerifier.create(deckService.update(document)).verifyError(NotFoundException.class);
        verify(deckQueryService).findById(any(String.class));
    ...
    @Test
    void whenTryToDeleteStoreDeckThenThrowError() {
        when(deckQueryService.findById(any(String.class))).thenReturn(Mono.error(new NotFoundException("")));
